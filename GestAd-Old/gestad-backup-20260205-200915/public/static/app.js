...
// Documents sync listeners: react to app-global notifications and localStorage changes
(function initDocumentsSyncListeners() {
  try {
    // transient last-seen timestamp to avoid duplicate reloads
    window._docsLastSeenUpdated = window._docsLastSeenUpdated || 0;

    function onDocumentsUpdatedTrigger() {
      try {
        // prefer a dedicated init if exists, otherwise renderDocs page 1
        if (typeof window.initDocs === 'function') {
          window.initDocs();
        } else if (typeof window.renderDocs === 'function') {
          window.renderDocs(1, '');
        } else {
          console.debug('documents sync: no initDocs/renderDocs available');
        }
      } catch (e) {
        console.debug('documents sync: onDocumentsUpdatedTrigger failed', e);
      }
    }

    // expose app-level notifier if app already uses such hook (no-op if already provided)
    if (typeof window.notifySectionUpdated !== 'function') {
      window.notifySectionUpdated = function(section) {
        if (section === 'documents') onDocumentsUpdatedTrigger();
      };
    }

    // BroadcastChannel: listen to app-global messages
    try {
      if ('BroadcastChannel' in window) {
        window._appGlobalBC = window._appGlobalBC || new BroadcastChannel('app-global');
        window._appGlobalBC.addEventListener('message', function(ev) {
          try {
            if (!ev) return;
            const data = ev.data;
            // support both simple string 'refresh' and object {type:'section:updated', section:'documents'}
            if (typeof data === 'string' && (data === 'refresh' || data === 'documents:refresh')) {
              onDocumentsUpdatedTrigger();
              return;
            }
            if (data && data.type === 'section:updated' && data.section === 'documents') {
              onDocumentsUpdatedTrigger();
              return;
            }
          } catch (e) {
            console.debug('documents sync: BC message handler error', e);
          }
        });
      }
    } catch (e) {
      console.debug('documents sync: BroadcastChannel init failed', e);
    }

    // storage event: listen for keys written by upload handlers
    window.addEventListener('storage', function(ev) {
      try {
        if (!ev) return;
        const key = ev.key;
        if (key === 'documentsUpdated' || key === 'docsStoreUpdated') {
          const v = Number(ev.newValue || 0);
          if (v && v > (window._docsLastSeenUpdated || 0)) {
            window._docsLastSeenUpdated = v;
            onDocumentsUpdatedTrigger();
          }
        }
      } catch (e) {
        console.debug('documents sync: storage handler error', e);
      }
    });

    // startup check: if other tab/browser wrote docsUpdated while we were closed, sync now
    try {
      const last = Number(localStorage.getItem('documentsUpdated') || localStorage.getItem('docsStoreUpdated') || 0);
      if (last && last > (window._docsLastSeenUpdated || 0)) {
        window._docsLastSeenUpdated = last;
        onDocumentsUpdatedTrigger();
      }
    } catch (e) {
      console.debug('documents sync: startup check failed', e);
    }

    console.debug('documents sync listeners installed');
  } catch (err) {
    console.debug('initDocumentsSyncListeners overall failed', err);
  }
})();
