...
(function () {
  ...
  // small util: detect legislation-like category (covers 'legislation' and 'législation' and variants)
  function isLegislationLike(doc) {
    try {
      if (!doc) return false;
      const src = (doc._source || '').toString().toLowerCase();
      if (src.indexOf('legis') !== -1) return true;
      const cat = (doc.category || '').toString().toLowerCase();
      if (cat.indexOf('legis') !== -1) return true;
      const kind = (doc.kind || '').toString().toLowerCase();
      if (kind.indexOf('legis') !== -1) return true;
      if (doc.isLegislation) return true;
    } catch (e) { /* ignore */ }
    return false;
  }
  ...
  window.loadPreviewContent = async function (docData) {
    ...
    const mimeType = (docData && docData.mime_type || '').toString().toLowerCase();

    // pick a safe download URL (prefer explicit _downloadUrl, otherwise select legislation vs documents endpoint)
    function computeDownloadUrl(d) {
      if (!d) return null;
      if (d._downloadUrl && String(d._downloadUrl).trim() !== '') return d._downloadUrl;
      if (isLegislationLike(d)) {
        return `/api/legislation/${encodeURIComponent(d.id)}/download`;
      }
      return `/api/documents/${encodeURIComponent(d.id)}/download`;
    }

    const downloadUrl = computeDownloadUrl(docData);
    ...
    if (mimeType.includes('pdf')) {
      ...
      if (prevBtn) {
        prevBtn.onclick = (e) => {
          // click handlers are not passive; keep preventDefault but ensure no passive listener wrapper is used
      ...
      // load pdf via pdf.js
      try {
        // If we have auth, fetch the bytes (so we can include Authorization).
        if (authHeader && Object.keys(authHeader).length > 0) {
          const resp = await fetch(downloadUrl, { headers: authHeader });
          ...
          // no auth header: let pdfjs fetch directly
        ...
        viewer.innerHTML = `
          <div style="padding:2rem;text-align:center;color:#333;">
            ...
            <p style="font-size:0.9rem;color:#666;">${err && err.message ? escapeHtml(String(err.message)) : ''}</p>
            <button class="preview-btn" onclick="window.downloadDocument(${JSON.stringify(docData.id)})">📥 Télécharger</button>
          </div>
        `;
      }
    ...
    // Images
      ...
      try {
        if (authHeader && Object.keys(authHeader).length > 0) {
          // fetch with auth, create blob URL so image request carries token
          const resp = await fetch(downloadUrl, { headers: authHeader });
          if (!resp.ok) throw new Error('Erreur chargement image: ' + resp.status);
          const blob = await resp.blob();
          const blobUrl = URL.createObjectURL(blob);
          viewer.innerHTML = `
            <img src="${blobUrl}" alt="${escapeHtml(docData.title || '')}" style="max-width:100%;max-height:100%;object-fit:contain;border-radius:8px;box-shadow:0 8px 32px rgba(0,0,0,0.5);">
          `;
        } else {
          // no auth required
          viewer.innerHTML = `
            <img src="${downloadUrl}" alt="${escapeHtml(docData.title || '')}" style="max-width:100%;max-height:100%;object-fit:contain;border-radius:8px;box-shadow:0 8px 32px rgba(0,0,0,0.5);">
          `;
        }
      } catch (err) {
        console.error('Erreur image:', err);
        viewer.innerHTML = `
          <div class="preview-error" style="color:#333;padding:2rem;text-align:center;">
            <div style="font-size:2rem;">⚠️</div>
            <div>Impossible de charger l'image</div>
            <div style="margin-top:1rem;">
              <a class="preview-btn" href="${downloadUrl}" target="_blank" rel="noopener">📥 Télécharger</a>
            </div>
          </div>
        `;
      }
    ...
    // Text
      ...
      try {
        ...
        if (!res.ok) throw new Error('Erreur téléchargement texte: ' + res.status);
        ...
        viewer.innerHTML = `<div style="padding:2rem;color:#333">Impossible de charger le texte</div>`;
      }
      return;
    }
      ...
      viewer.innerHTML = `
        <div class="preview-unsupported">
          ...
          <div>
            ...
            <div class="preview-unsupported-subtext">Type: ${escapeHtml(mimeType || 'Inconnu')}</div>
          </div>
          <a class="preview-btn" href="${downloadUrl}" target="_blank" rel="noopener" style="margin-top:1rem;">📥 Télécharger</a>
        </div>
      `;
      return;
    }
