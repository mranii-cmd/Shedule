...
router.put('/:id/read', jwtAuth, async (req, res, next) => {
  try {
    ...
    if (io) io.to(`user_${userId}`).emit('notification:updated', { id: Number(id), read: true });
 ...
 */
router.delete('/:id(\\d+)', jwtAuth, async (req, res, next) => {
  try {
    const db = req.app.get('knex');
    const io = req.app.get('io');
    const { id } = req.params;
...
// Supprime les notifications LUES de l'utilisateur courant (suppression par lots, robuste)
router.delete('/read', jwtAuth, async (req, res, next) => {
  try {
    const knex = req.app.get('knex');
    const io = req.app.get('io');
    const userId = req.user.id;

    console.info(`[notifications] user ${userId} requested DELETE /read`);
    ...
    while (true) {
      const rows = await knex('notifications')
        .where('user_id', userId)
        .andWhere(function () {
          // match les représentations courantes : true OR 1
          this.where('read', true).orWhere('read', 1);
        })
        .limit(batchSize)
        .select('id');
      ...
      const deleted = await knex('notifications').whereIn('id', ids).del();
      totalDeleted += deleted;
    }

    if (io) io.to(`user_${userId}`).emit('notification:deleted:read', { deleted: totalDeleted });

    console.info(`[notifications] user ${userId} deleted ${totalDeleted} read notifications`);
...
/**
 * DELETE /api/notifications/unread
 * - Supprime toutes les notifications non lues de l'utilisateur courant.
 */
router.delete('/unread', jwtAuth, async (req, res, next) => {
  try {
    const knex = req.app.get('knex');
    const userId = req.user.id;
    ...
    const io = req.app.get('io');
    if (io) io.to(`user_${userId}`).emit('notification:deleted:unread', { deleted });

    res.json({ ok: true, deleted });
  } catch (err) {
    console.error('Erreur suppression notifications non lues:', err);
    next(err);
  }
});
